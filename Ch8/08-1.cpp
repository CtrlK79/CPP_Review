...
namespace RISK_LEVEL {enum {RISK_A, RISK_B, RISK_C};}
...
private:
    ...
    int payPerHour;
public:
    TemporaryWorker(const char* name, const int pay) : Employee(name), workTime(0), payPerHour(pay) {}
    void AddWorkTime(const int time) {workTime+=time;}
    int GetPay() const {return workTime*payPerHour;}
    void ShowSalaryInfo() const {ShowYourName(); cout<<"salary: "<<GetPay()<<endl<<endl;}
};
...
public:
    ...
    void AddSalesResult(const int value) {salesResult+=value;}
...
private:
    int risk;
public:
    ForeignSalesWorker(const char* name, const int money, const double ratio, const int risk)
